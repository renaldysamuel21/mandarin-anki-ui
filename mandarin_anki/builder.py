    ...
    *,
    ...
    def ensure_tts() -> TTSLike:
        ...
        if tts_model is None:
            ...
            except ImportError as exc:  # pragma: no cover - depends on external wheels
                raise DeckBuildError(
                    "Model TTS gagal dimuat. Pastikan numpy dan dependensi audio terinstal (lihat bagian troubleshooting README)."
                ) from exc
            except Exception as exc:  # pragma: no cover - defensive guardrail
                raise DeckBuildError(f"Model TTS gagal dimuat: {exc}") from exc
            ...
            if last_error is not None:
                raise DeckBuildError(
                    "Model TTS tidak dapat dijalankan pada perangkat mana pun. Periksa instalasi CUDA/CPU dan ulangi."
                ) from last_error
