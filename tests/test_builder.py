...
from mandarin_anki import DeckBuildConfig, DeckBuildError, build_anki_deck
...
def test_build_anki_deck_surfaces_import_errors(tmp_path):
    csv_path = tmp_path / "deck.csv"
    csv_path.write_text("Hanzi,Pinyin,Indo\n好,hǎo,baik\n", encoding="utf-8")

    speaker_wav = tmp_path / "speaker.wav"
    _build_wav(speaker_wav)

    class _FailFactory:
        def create(self, model_name: str):  # pragma: no cover - trivial stub
    ...
    with pytest.raises(DeckBuildError) as excinfo:
        build_anki_deck(config, tts_factory=_FailFactory())
